...
class ClientModel:
    ...
    def create(self, client: Client):
        ...
        struct = PointStruct(id=id, payload=payload, vector=embedding)
    ...
    def chat(self, chat: Chat) -> Chat:
        ...
        if (
            len(chat.messages) == 1
        ):  # If the chat just started, and user sent his first query
            messages.append(
                {
                    "role": "user",
                    "parts": [
        ...
        newChat = Chat(title=title, created=created, messages=newMessages)
    ...
    def update(self, id: int, chat: Chat) -> ClientQdrant:
        ...
        qdrant_connection.set_payload(
            collection_name=self.collection_name,
            payload={"history": client.history},
            points=[id],
        )
